...
type Title = { id:number; name:string; description:string|null; cover_url:string|null; type:string; total_seasons:number|null; total_episodes:number|null; year:number|null; genres:string[]|null; admin_only:boolean|null }
...
export default async function TitlePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Bloqueia acesso direto pela URL se não for admin
  ...
  // Filtra similares que também são admin_only se não for admin
  const typedSimilar = ((similar ?? []) as SimilarTitle[]).filter((t: any) => isAdmin || !t.admin_only)
